...
//左右切页面
  var details=(function(){
      ...
      function show(index){
          ...
          $li.eq(index).show();
          $detailsReturn.show();
          $detailsBtn.find("li").eq(index).addClass("active").siblings().removeClass("active");
          $li.eq(index).attr("class","fadeIn");
          iNow=index;
      };
      function hide(iNow){
          $cube.show();
          $li.eq(iNow).attr("class","fadeOut");
          $detailsReturn.hide();
          $li.eq(iNow).one('animationEnd webkitAnimationEnd',function(){
              $details.hide();
              $li.attr("class","");
          })
      }
